...
export default function Reviews() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader testid="reviews-header" title="Review Queue" subtitle="The human-in-the-loop queue. Every action is recorded to the audit trail.">
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent>
            ...
            <SelectItem value="Context Requested">Context Requested</SelectItem>
            <SelectItem value="Context Received">Context Received</SelectItem>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-xs text-muted-foreground border-b border-border bg-muted/30">
                ...
                <th className="font-medium py-2.5 px-4">Assigned reviewer</th>
              ...
              <tbody>
                {data.reviews.map((r) => (
                  <tr key={r.id} data-testid={`review-row-${r.id}`} onClick={() => nav(`/reviews/${r.id}`)}
                    ...
                    <td className="py-2.5 px-4"><ConfidenceBadge confidence={r.confidence} /></td>
